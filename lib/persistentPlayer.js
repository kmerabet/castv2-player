...
module.exports = function (logClass) {
  ...
  var CustomMediaReceiver   = require('castv2-client').CustomMediaReceiver;
  ...
  class PersistentPlayer extends EventEmitter {
    ...
    playUrl (url, options, title) {
      ...
      return that._playlist.addUrlPromise(url, options, title)
      .catch (function(err) {
    ...
    getDefaultPlayerPromise () {
      ...
      //Is the Media Receiver already loaded?
      if (that._player && that._currentApplicationId  == CustomMediaReceiver.APP_ID){
        return Promise.resolve(that);
      }
      
      //detach and launch the Media Receiver
      that._detachPlayer();
      
      log.info("%s - Try to launch the Receiver", that._name);
      return that._client.launchPromise(CustomMediaReceiver)
      .then (function (p) {      
        that._currentApplicationId = CustomMediaReceiver.APP_ID;
        
        that._playerConnected(p);
        log.info("%s - Media Receiver lauched", that._name);;
