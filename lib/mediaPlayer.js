...
module.exports = function (logClass) {
  ...
  class MediaPlayer extends EventEmitter {
    ...
    createVirtualApp (session) {
      this._application = new Application(this._client._client.client, session);
    }

    //playURL()
    playUrlPromise (url, options, title) {
      return this._player.playUrl(url, options, title);
    }
