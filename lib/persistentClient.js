...
module.exports = function (logClass) {
  ...
  //Constants
  const MAX_CONECTIONS_RETRIES = 10;
  ...
  class PersistentClient extends EventEmitter {
    ...
    //send alert
    _alertPromise(message){
      let that = this;
      return new Promise (function (resolve, reject) {
        log.info("%s - Trying to send message: %s", that._name, message);
        //that.client.send("sender-0", "receiver-0", "urn:x-cast:com.sii.cast.messaging", message);
        resolve();
      })
    }
